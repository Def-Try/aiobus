...
class basic(commands.Cog):
    ...
    async def help_general(self, ctx: discord.ApplicationContext):
        ...
        def do_commands(ctx, commands):
            ...
            if len(commands[5:]) > 0:
                ...
                ).format(amount=len(commands[5:]))
            return string.strip()

        for name, cog in self.bot.cogs.items():
            embed.add_field(
                name=f"Cog `{name}` - {localise(f'cog.{cog.qualified_name}.info.brief', ctx.interaction.locale)}",
    ...
    async def help_cog(self, ctx: discord.ApplicationContext, cog):
        ...
        embed.add_field(
            name=f"Commands", value=do_commands(ctx, cog.get_commands()), inline=False
        )
        await ctx.respond(embed=embed, ephemeral=True)
    ...
    async def ping(self, ctx: discord.ApplicationContext):
        ...
        embed.add_field(
            ...
            value=f"{round(self.bot.latency*1000, 2)}{localise('cog.basic.answers.ping.unit', ctx.interaction.locale)}",
            inline=False,
        )
