...
class starboard(commands.Cog):
    ...
    async def init(self, ctx: discord.ApplicationContext, channel: discord.Option(discord.TextChannel,
        ...
        if self.db.search(Query().channel == channel.id):
            ...
            return
        self.db.insert(
            {"channel": channel.id, "guild": channel.guild.id, "limit": limit, "messages": {}})
        await ctx.respond(localise("cog.starboard.answers.init.ok", ctx.interaction.locale))

    @cmds.command(guild_ids=CONFIG["g_ids"],
                  name_localizations=localise(
                      "cog.starboard.commands.destroy.name"),
    ...
    async def starboard_react(self, payload):
        ...
        if not starboards:
            return
        stars = list(filter(lambda rc: rc.emoji ==
                     self.emoji, message.reactions))
        if len(stars) == 0:
            star_amount = 0
        else:
        ...
        for starboard in starboards:
            ...
            embed.set_author(
                name=message.author.name if not hasattr(
                    message.author, "nick") or not message.author.nick else message.author.nick,
                icon_url=message.author.avatar.url if message.author.avatar else message.author.default_avatar.url
            )
            embeds = [embed] + \
                message.embeds if not message.channel.nsfw else []
