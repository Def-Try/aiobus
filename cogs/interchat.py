...
class interchat(commands.Cog, name="interchat"):
    ...
    async def complete_init_interchat(self):
        for tunnel in self.tdb:
            ...
            rtunnel["out"] = self.bot.get_channel(
                tunnel["out"]) or self.bot.get_partial_messageable(tunnel["out"])
            rtunnel["in"] = self.bot.get_channel(
                tunnel["in"]) or self.bot.get_partial_messageable(tunnel["in"])
        ...
        if addr is None:
            while True:
                ...
                if len(addrcs1) == 0 and len(addrcs2) == 0:
                    break
            self.db.insert({"chid": channel.id, "address": addr, "guid": channel.guild.id if not isinstance(
                channel, discord.abc.PrivateChannel) and not isinstance(channel, discord.PartialMessageable) else -1})
        return addr
        ...
        if addr is None and create:
            while True:
                ...
                if len(addrcs1) == 0 and len(addrcs2) == 0:
                    break
            self.hdb.insert(
                {"channels": [], "address": addr, "host": channel.id})
    ...
    async def start_interchat(self, fromch, toch, hub=None):
        ...
        whookless = isinstance(fromch, discord.abc.PrivateChannel) or isinstance(toch, discord.abc.PrivateChannel) or isinstance(
            fromch, discord.PartialMessageable) or isinstance(toch, discord.PartialMessageable)
    ...
    async def bind_interchat_hub(self, hub_addr, channel):
        ...
        if await self.start_interchat(channel, self.bot.get_channel(hub["host"]), hub_addr):
            ...
            self.hdb.update(
                {"channels": hub["channels"] + [channel.id]}, Query().address == hub_addr)
            return 1
        else:
            return -3
    ...
    async def end_interchat(self, tunnel):
        if not tunnel["whookless"] and not tunnel["hub_addr"]:
            ...
            if not hub or (hub and len(hub["channels"]) == 1):
                ...
                await tunnel["inwhook"].delete()
        q = Query()
        self.tdb.remove(q["in"] == tunnel["in"].id and q["out"] ==
                        tunnel["out"].id and q["hub_addr"] == tunnel["hub_addr"])
    ...
    async def unbind_interchat_hub(self, hub_addr, channel):
        ...
        self.hdb.update({"channels": list(filter(
            lambda ch: ch != channel.id, hub["channels"]))}, Query().address == hub_addr)
        return 1
    ...
    async def tunneling_onmsg(self, message):
        if message.author.id == self.bot.user.id or message.author.id in interchat_bans["send"]:
            return
        for itunnel in self.tunnels:
            if not itunnel["whookless"] and (message.author.id == itunnel["outwhook"].id or message.author.id == itunnel["inwhook"].id):
                return
            if not (message.channel.id == itunnel["out"].id or message.channel.id == itunnel["in"].id):
                continue
            embeds = []
            if message.reference:
                ...
                embeds.append(
                    discord.Embed(
                        title=localise(
                            "cog.interchat.answers.reply", DEFAULT_LOCALE),
                        description=(resolved.content
                                     if resolved.content and len(resolved.content) < 30 else
                                     resolved.content[:27]+"..."
                                     if resolved.content else
                                     discord.Embed.Empty)
                    )
                )
                embeds[-1].set_author(
                    name=(resolved.author.name
                          if isinstance(resolved.author, discord.User) or not resolved.author.nick else
                          resolved.author.nick),
                    icon_url=(resolved.author.avatar.url
                              if resolved.author.avatar else
                              resolved.default_avatar.url
                              )
                )
            ...
            if not itunnel["whookless"]:
                channel = itunnel["inwhook" if message.channel.id ==
                                  itunnel["out"].id else "outwhook"]
            else:
                channel = itunnel["in" if message.channel.id ==
                                  itunnel["out"].id else "out"]
            if not itunnel["whookless"]:
                sent_message = await channel.send(
                    message.content,
                    username=(message.author.name
                              if isinstance(message.author, discord.User) or not message.author.nick else
                              message.author.nick) + ((" | "+self.address_string(message.channel)) if itunnel["hub_addr"] else ""),
                    avatar_url=(
                        message.author.avatar.url if message.author.avatar else message.author.default_avatar.url),
                    embeds=embeds,
    ...
    async def tunneling_onmsgedit(self, message):
        ...
        for itunnel in self.tunnels:
            ...
            sent_message = None
            old_message = itunnel["messages"][itunnel["rmessages"].index(
                message.id)]
            channel = None
            if not itunnel["whookless"]:
                channel = itunnel["inwhook" if message.channel.id ==
    ...
    async def tunneling_onmsgdel(self, message, message_is_id=False, chid=None):
        if not message_is_id and (message.author.id == self.bot.user.id or message.author.id in interchat_bans["send"]):
            return
        if message_is_id:
            class FakeObject:
    ...
    @cmds.command(guild_ids=CONFIG["g_ids"],
                  name_localizations=localise(
                      "cog.interchat.commands.bind.name"),
    ...
    async def bind(self, ctx: discord.ApplicationContext, address: discord.Option(str,
        ...
        if result == 1:
            ...
            embed.add_field(inline=False, name=localise(
                "cog.interchat.answers.info.started", DEFAULT_LOCALE), value=f"<t:{round(time.time())}:F>")
            embed.add_field(inline=False, name=localise("cog.interchat.answers.info.our_side", DEFAULT_LOCALE), value=localise(
                "cog.interchat.answers.info.receiver_side", DEFAULT_LOCALE))
            embed.add_field(inline=False, name=localise("cog.interchat.answers.info.hub.incoming_to",
                            DEFAULT_LOCALE), value=self.address_string_hub(self.bot.get_channel(hub["host"])))
            embed.add_field(inline=False, name=localise(
                "cog.interchat.answers.info.outgoing_from", DEFAULT_LOCALE), value=self.address_string(ctx.channel))
    ...
    @cmds.command(guild_ids=CONFIG["g_ids"],
                  name_localizations=localise(
                      "cog.interchat.commands.begin.name"),
    ...
    async def begin(self, ctx: discord.ApplicationContext, address: discord.Option(str,
        ...
        channel = self.bot.get_channel(
            chid) or self.bot.get_partial_messageable(chid)
        ...
        if await self.start_interchat(ctx.channel, channel):
            ...
            embed.add_field(inline=False, name=localise("cog.interchat.answers.info.our_side", DEFAULT_LOCALE), value=localise(
                "cog.interchat.answers.info.receiver_side", DEFAULT_LOCALE))
            embed.add_field(inline=False, name=localise(
                "cog.interchat.answers.info.incoming_to", DEFAULT_LOCALE), value=self.address_string(channel))
            embed.add_field(inline=False, name=localise(
                "cog.interchat.answers.info.outgoing_from", DEFAULT_LOCALE), value=self.address_string(ctx.channel))
            embed.color = discord.Color.green()
    ...
    @cmds.command(guild_ids=CONFIG["g_ids"],
                  name_localizations=localise(
                      "cog.interchat.commands.unbind.name"),
    ...
    @cmds.command(guild_ids=CONFIG["g_ids"],
                  name_localizations=localise(
                      "cog.interchat.commands.destroy_hub.name"),
    ...
    async def destroy_hub(self, ctx: discord.ApplicationContext):
        ...
        for channel in hub["channels"]:
            ...
            embed.add_field(inline=False, name=localise("cog.interchat.answers.info.hub.incoming_to",
                            DEFAULT_LOCALE), value=self.address_string_hub(self.bot.get_channel(hub["host"])))
            embed.add_field(inline=False, name=localise("cog.interchat.answers.info.outgoing_from",
                            DEFAULT_LOCALE), value=self.address_string(self.bot.get_channel(channel)))
            embed.color = discord.Color.green()
            await self.bot.get_channel(channel).send(embed=embed)
    ...
    @cmds.command(guild_ids=CONFIG["g_ids"],
                  name_localizations=localise(
                      "cog.interchat.commands.end.name"),
        ...
        for i, tunnel in enumerate(self.tunnels):
            if ctx.channel.id == tunnel["in"].id or ctx.channel.id == tunnel["out"].id:
                ...
                embed.title = localise("cog.interchat.answers.end.incoming", DEFAULT_LOCALE).format(
                    side=(localise("cog.interchat.answers.info.receiver_side", DEFAULT_LOCALE) if ctx.channel.id ==
                          tunnel["in"].id else localise("cog.interchat.answers.info.opener_side", DEFAULT_LOCALE))
                )
                embed.add_field(inline=False, name=localise(
                    "cog.interchat.answers.info.started", DEFAULT_LOCALE), value=f"<t:{tunnel['started']}:F>")
                embed.add_field(inline=False, name=localise("cog.interchat.answers.info.our_side", DEFAULT_LOCALE), value=localise(
                    "cog.interchat.answers.info.receiver_side", DEFAULT_LOCALE))
                embed.add_field(inline=False, name=localise(
                    "cog.interchat.answers.info.incoming_to", DEFAULT_LOCALE), value=self.address_string(tunnel["in"]))
                embed.add_field(inline=False, name=localise(
                    "cog.interchat.answers.info.outgoing_from", DEFAULT_LOCALE), value=self.address_string(tunnel["out"]))
                embed.color = discord.Color.red()
                await tunnel["in"].send(embed=embed)

                embed = discord.Embed()
                embed.title = localise("cog.interchat.answers.end.outgoing", DEFAULT_LOCALE).format(
                    side=(localise("cog.interchat.answers.info.opener_side", DEFAULT_LOCALE) if ctx.channel.id ==
                          tunnel["out"].id else localise("cog.interchat.answers.info.receiver_side", DEFAULT_LOCALE))
                )
                embed.add_field(inline=False, name=localise(
                    "cog.interchat.answers.info.started", DEFAULT_LOCALE), value=f"<t:{tunnel['started']}:F>")
                embed.add_field(inline=False, name=localise("cog.interchat.answers.info.our_side", DEFAULT_LOCALE), value=localise(
    ...
    @cmds.command(guild_ids=CONFIG["g_ids"],
                  name_localizations=localise(
                      "cog.interchat.commands.address.name"),
    ...
    @cmds.command(guild_ids=CONFIG["g_ids"],
                  name_localizations=localise(
                      "cog.interchat.commands.info.name"),
    ...
    async def info(self, ctx: discord.ApplicationContext):
        ...
        for i, tunnel in enumerate(self.tunnels):
            if ctx.channel == tunnel["in"] or ctx.channel == tunnel["out"]:
                this_tunnel = tunnel
                break
        embed = discord.Embed(title=localise(
            "cog.interchat.answers.info.title", ctx.interaction.locale))
        if not this_tunnel:
        ...
        embed.add_field(inline=False, name=localise("cog.interchat.answers.info.started",
                        ctx.interaction.locale), value=f"<t:{this_tunnel['started']}:F>")
        embed.add_field(inline=False, name=localise("cog.interchat.answers.info.our_side", ctx.interaction.locale), value=(localise("cog.interchat.answers.info.receiver_side",
                        ctx.interaction.locale) if ctx.channel.id == this_tunnel["in"].id else localise("cog.interchat.answers.info.opener_side", ctx.interaction.locale)))
        embed.add_field(inline=False, name=localise("cog.interchat.answers.info.incoming_to",
                        ctx.interaction.locale), value=self.address_string(this_tunnel["in"]))
        embed.add_field(inline=False, name=localise("cog.interchat.answers.info.outgoing_from",
                        ctx.interaction.locale), value=self.address_string(this_tunnel["out"]))
        embed.add_field(inline=False, name=localise("cog.interchat.answers.info.permanent", ctx.interaction.locale), value=(localise(
            "generic.yes", ctx.interaction.locale) if this_tunnel["permanent"] else localise("generic.no", ctx.interaction.locale)))
