...
#        )
         udata = self.udata.get(str(ctx.author.id), [[{"role": "system", "content": self.template}], list(self.laws)])
         ...
         for provider in [
             ...
             if result and result != "Hmm, I am not sure. Email support@chatbase.co for more info.": break
         if not result: result = "Errored: no provider responded with vaid answer... Try again later?"
