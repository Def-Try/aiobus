...
class mnlcog(commands.Cog):
    ...
    async def configure(self, ctx: discord.ApplicationContext, cfgname: discord.Option(str, choices=['persistent']), cfgval: str):
        ...
        if cfgname == "persistent":
            ...
            if engine.persisting_globals:
                ...
                await ctx.respond(localise("cog.mnlcog.answers.config.persisting_globals.false", ctx.interaction.locale))
            return
    ...
    async def run(self, ctx: discord.ApplicationContext, code: str):
        ...
        try:
            ...
            await ctx.followup.send(localise(f"cog.mnlcog.answers.run.mnl_error.{(('with' if output else 'no')+'_output')}", ctx.interaction.locale).format(error=str(e), output=output))
            return
        except TimeoutError as e:
            output = engine.fio.read_output()
            await ctx.followup.send(localise(f"cog.mnlcog.answers.run.timed_out.{(('with' if output else 'no')+'_output')}", ctx.interaction.locale).format(timeout=str(timeout), output=output))
            return
        except Exception as e:
            await ctx.followup.send(localise(f"cog.mnlcog.answers.run.fatal_error.{(('with' if output else 'no')+'_output')}", ctx.interaction.locale).format(error=str(e), output=output))
            return
        output = engine.fio.read_output()
        await ctx.followup.send(localise(f"cog.mnlcog.answers.run.ok.{(('with' if output else 'no')+'_output')}", ctx.interaction.locale).format(output=output))
